...
- Stack: **xUnit v3** (`xunit.v3` package, not `xunit`) running under the **Microsoft Testing Platform** runner (`UseMicrosoftTestingPlatformRunner=true`), not VSTest. There is exactly one unit test project per bounded context, named `{Ctx}.Tests` (e.g. `Hiscary.UserAccounts.Tests`) — it covers every layer of that context, organized into subfolders that mirror the layer under test (`Domain/`, `Application/`, etc.), not a separate project per layer.
...
- Unit tests target `{Ctx}.Domain` and `{Ctx}.Application.*` types in isolation — no `DbContext`, no HTTP, no Aspire orchestration. Domain-layer tests live under `{Ctx}.Tests/Domain/` and reference only `{Ctx}.Domain`; application-layer tests live under `{Ctx}.Tests/Application/` and reference the relevant `{Ctx}.Application.*` project(s), all within the same `{Ctx}.Tests` project. Anything that needs a real Postgres/RabbitMQ/HTTP pipeline belongs in `{Ctx}.IntegrationTests` instead (see `backend/integration-testing.md`).
