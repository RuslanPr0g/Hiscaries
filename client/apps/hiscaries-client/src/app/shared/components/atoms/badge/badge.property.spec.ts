...
describe('BadgeComponent — Property 1: variant always produces a CSS class', () => {
  ...
  it('should apply a non-empty variant CSS class for every value in the union', () => {
    const variantArb = fc.constantFrom('default', 'accent', 'danger', 'success') as fc.Arbitrary<
      'default' | 'accent' | 'danger' | 'success'
    >;
    ...
    fc.assert(
      fc.property(variantArb, (variant) => {
        ...
        const allVariants: ('default' | 'accent' | 'danger' | 'success')[] = [
          'default',
          'accent',
          'danger',
          'success',
